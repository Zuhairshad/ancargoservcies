...
export class PostgresStore implements ShipmentStore {
  ...
  async create(input: NewShipment): Promise<Shipment> {
    return this.withTransaction(async (client) => {
      ...
      await client.query(
        `insert into shipment_events (ref, status, at, location, note) values ($1, 'booked', $2, $3, $4)`,
...
export function createPool(connectionString: string) {
  ...
  if (process.env.NODE_ENV !== 'production') globalForPool.ancsAdminPool = pool
  return pool
}
